...
_ssize_t _read_r(struct _reent *r, int file, void *ptr, size_t len)
{
  ...
  for (i = 0; i < len; i++)
  {
    ...
    *p++ = c;
//    uartPutch(c);
    SemihostCall(Semihost_SYS_WRITEC, &c);                    // not sure?

    if (c == 0x0D && i <= (len - 2))
    {
      *p = 0x0A;
//      uartPutch(0x0A);
      SemihostCall(Semihost_SYS_WRITEC, &(*p) );
      return i + 2;
    }
  }
  return i;
    ...
    size_t len)
{
    ...
    const unsigned char *new_return = "\r";
	
	p = (const unsigned char*) ptr;
	
	for (i = 0; i < len; i++) {
//		if (*p == '\n' ) uartPutch('\r');
		if (*p == '\n' ) SemihostCall(Semihost_SYS_WRITE0, (void*)new_return );
//		uartPutch(*p++);
        SemihostCall(Semihost_SYS_WRITEC, (void*)&(*p++) );
	}
